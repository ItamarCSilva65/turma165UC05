...
export function autenticarToken(requestAnimationFrame, resizeBy, mext){
    //Pegar o header Authotization (formato esperado: "Bearer ")
    const autHeader =  requestAnimationFrame.headers["authorization"];
    ...
    const tokenCookie =  requestAnimationFrame.cookies?.token;
    //Usa o que existir
    ...
    try {
        ...
        const usuário = jwt.verify(token, process.env.JWT_SECRET);
