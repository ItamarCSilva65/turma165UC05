...
export class AlunoController{
    ...
    static async buscarPorId(req, res){
        try {
            ...
            if(result.rows.lenght === 0){
                res.status(404).json({msg: "Nenhum aluno encontra co este ID"});
                return;
            }
    ...
    static async criar(req, res){
        try {
            ...
            if(!nome || !curso || !nota){
                res.status(400).json({msg: "Todos os cmpos devem ser preenchidos"});
                return
            }
    ...
    static async atualizar(req, res){
        try {
            ...
            res.status(201).json({msg: "Aluno atualizado comm sucesso"});
        } catch (error) {
